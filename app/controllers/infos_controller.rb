class InfosController < ApplicationController
  ...
  # GET /infos.json
  def index
    @infos = Info.user_infos(current_user)
  end
  ...
  # POST /infos.json
  def create
    #@info = @song.infos.new(info_params)
  ...
  private
    def set_song
      @song = Song.find_by(id: params[:song_id]) || Song.find(info_params[:song_id])
    end
    # Use callbacks to share common setup or constraints between actions.
    def set_info
      @info = Info.find(params[:song_id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def info_params
      params.require(:info).permit(:song_id, :title)
    end
end
